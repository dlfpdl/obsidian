...
기획자는 없었다. PO 한명으로 충분하지 않은가? UX에 대하여 얼마나 전문가인가. 승희 R&R: PM(Task Planning & Review) & PO(목표 KPI, 가설-검증, 로드맵 설계, 방향성 모색: Core Value를 기반으로 이를 구현할 수 있는 모든 기능에 대해서 브레인스톰, 설계를 한다.)[[스크럼 가이드#PO]]